...
def callback(msg):
	...
	for i in range(5):
		pulses[i] = (int)(abs(msg.data[i]-angles[i])*PPR[i]*GEAR_RATIO[i])
		if(msg.data[i]>angles[i]):
			dir_ena = dir_ena|(1<<i)
		if(pulses[i]>MAX_PULSES):
			pulses[i] = MAX_PULSES
			rospy.logwarn('Speed limit exceeded for motor %d',(i+1))
			del_ang = math.copysign(MAX_PULSES/(PPR[i]*GEAR_RATIO[i]),msg.data[i]-angles[i])
...
def listener():
	...
	rospy.Subscriber('joint_angles', Float32MultiArray, callback)
	rospy.spin()
